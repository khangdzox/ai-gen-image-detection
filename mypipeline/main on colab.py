...
CONFIG_ATTRIBUTES_REQUIRE_DENOISE = {'train_samples', 'test_samples', 'hf_repo', 'timesteps', 'diffusion_percent'}
...
def extract_noise_features(
    ...
    assert all([f in ['noise_stats', 'noise_fft', 'residual_stats', 'cos_sim'] for f in included_features]), "included_features must be one of ['noise_stats', 'noise_fft', 'residual_stats', 'cos_sim']"
    assert not (noise is None and ('residual_stats' in included_features or 'cos_sim' in included_features)), "residual_stats and cos_sim require noise"
    ...
    for i in range(pred_noise.shape[0]):
        ...
        if 'noise_fft' in included_features:
            ...
            features += [pred_fft_magnitude, pred_fft_phase, pred_fft_low_energy, pred_fft_mid_energy, pred_fft_high_energy]
        ...
        if 'residual_stats' in included_features and residual is not None:
            ...
            features += [residual_mean, residual_std, residual_skew, residual_kurtosis, residual_l2]
...
class MyPipeline:
    ...
    def __init__(self, vae, unet, tokenizer, text_encoder, scheduler, total_timesteps=30, diffusion_percent=0.5, device='cuda'):
        """
    ...
    def __call__(self, batch: torch.Tensor):
        ...
        pred_noises_list = []
        for t, lats in tqdm(zip(self.scheduler.timesteps[self.t_start:], latents_list), total=len(latents_list), desc="Denoising", leave=False):
            with torch.no_grad():
    ...
    def extract_features(self, pred_noises_list, noises_list, included_features=['noise_stats', 'noise_fft', 'residual_stats', 'cos_sim']):
        """
...
class TransformerClassifier(torch.nn.Module):
    def __init__(self, input_size, hidden_size=64, num_layers=4, num_classes=2):
        ...
        encoder_layer = torch.nn.TransformerEncoderLayer(d_model=hidden_size, nhead=8, batch_first=True)
        ...
        for split in ['train', 'val']:
            for subdir, newname in [('nature', '0_real'), ('ai', '1_fake')]:
                ...
                for file in os.listdir(f'{datapath}/{model_dir}/{split}/{subdir}'):
                    try:
                        os.symlink(f'{datapath}/{model_dir}/{split}/{subdir}/{file}', f'data/{split}/{model_dir}/{newname}/{file}')
    ...
    if num_samples > 0:
        ...
        for model_dir in os.listdir(datapath):
            for split in ['train', 'val']:
                for subdir, newname in [('nature', '0_real'), ('ai', '1_fake')]:
                    ...
                    os.makedirs(f'data_{num_samples}/{split}/{model_dir}/{newname}', exist_ok=True)

                    random.seed(42)  # For reproducibility
                    random_files = random.sample(os.listdir(f'{datapath}/{model_dir}/{split}/{subdir}'), (num_train_samples if split == 'train' else num_test_samples)//2)

                    for file in random_files:
                        try:
                            os.symlink(f'{datapath}/{model_dir}/{split}/{subdir}/{file}', f'data_{num_samples}/{split}/{model_dir}/{newname}/{file}')
                        except FileExistsError:
                            pass
        ...
        for split in ['train', 'val']:
            for model_dir in os.listdir(f'data_{num_samples}/{split}'):
                for class_dir in ['0_real', '1_fake']:
                    os.makedirs(f'data_aio_{num_samples}/{split}/{class_dir}', exist_ok=True)

                    for file in os.listdir(f'data_{num_samples}/{split}/{model_dir}/{class_dir}'):
                        try:
                            os.symlink(
                                os.readlink(f'data_{num_samples}/{split}/{model_dir}/{class_dir}/{file}'),
                                f'data_aio_{num_samples}/{split}/{class_dir}/{model_dir}_{file}')
                        except FileExistsError:
                            pass

    return 'data', (f'data_{num_samples}' if num_samples > 0 else None), 'data_aio', (f'data_aio_{num_samples}' if num_samples > 0 else None)

def prepare_pipeline(hf_repo="CompVis/stable-diffusion-v1-4", total_timesteps=30, diffusion_percent=0.5, device='cuda'):
    ...
    """
    logger.info(f"Preparing pipeline with repo {hf_repo}, total_timesteps={total_timesteps}, diffusion_percent={diffusion_percent}, device={device}")
    ...
    return MyPipeline(vae, unet, tokenizer, text_encoder, scheduler, total_timesteps, diffusion_percent, device)
...
def run_pipeline_extract_features(pipeline: MyPipeline, all_pred_noises_and_noises, included_features):
    ...
    try:
        for pred_noises_list, noises_list in all_pred_noises_and_noises:
            features = pipeline.extract_features(pred_noises_list, noises_list, included_features)
...
def train_classifier(classifier, X_train, y_train, X_val, y_val, batch_size=8, epochs=50, lr=1e-3, weight_decay=1e-5, device='cuda'):
    ...
    logger.info(f"Training classifier with batch_size={batch_size}, epochs={epochs}, lr={lr}, weight_decay={weight_decay}, device={device}")
    ...
    for epoch in range(epochs):
        ...
        logger.info(f"Epoch {epoch+1:02}: Loss = {running_loss / total:.4f}, Accuracy = {correct / total:.4f}, Val Loss = {val_running_loss / val_total:.4f}, Val Accuracy = {val_correct / val_total:.4f}, LR = {scheduler.get_last_lr()[0]:.4f}{', New best' if is_best else ''}")

    assert best_model_state_dict is not None, "Training failed, either epochs == 0 or data is empty. No model was trained."
...
def normalise_features(features):
    ...
    return (features - mean[None, None, :]) / (std[None, None, :] + 1e-8)  # Add small value to avoid division by zero

def get_classifier(classifier_type, input_size, hidden_size=64, num_layers=1, num_classes=2):
    match classifier_type.lower():
        ...
        case 'transformer':
            return TransformerClassifier(input_size, hidden_size, num_layers, num_classes)
        case _:
...
def run_denoise(config, train_dataloader, val_dataloader):
    ...
    train_all_pred_noises_and_noises, train_all_labels = run_pipeline_denoise(pipeline, train_dataloader)
    val_all_pred_noises_and_noises, val_all_labels = run_pipeline_denoise(pipeline, val_dataloader)

    return pipeline, train_all_pred_noises_and_noises, train_all_labels, val_all_pred_noises_and_noises, val_all_labels

def run_extract_features_and_evaluate(config, pipeline, train_all_pred_noises_and_noises, train_all_labels, val_all_pred_noises_and_noises, val_all_labels, test_all_pred_noises_and_noises=None, test_all_labels=None):
    # Extract features
    included_features = config["included_features"]

    train_features = run_pipeline_extract_features(pipeline, train_all_pred_noises_and_noises, included_features)
    ...
    train_features_ts = (train_features_ts - train_features_mean) / (train_features_std + 1e-8)  # Add small value to avoid division by zero

    val_features = run_pipeline_extract_features(pipeline, val_all_pred_noises_and_noises, included_features)
    val_features_ts = torch.stack(val_features)
    val_features_ts = (val_features_ts - train_features_mean) / (train_features_std + 1e-8)  # Add small value to avoid division by zero
    val_labels_ts = torch.concat(val_all_labels)

    if test_all_pred_noises_and_noises is not None and test_all_labels is not None:
        test_features = run_pipeline_extract_features(pipeline, test_all_pred_noises_and_noises, included_features)
        test_features_ts = torch.stack(test_features)
        test_features_ts = (test_features_ts - train_features_mean) / (train_features_std + 1e-8)  # Add small value to avoid division by zero
        test_labels_ts = torch.concat(test_all_labels)
    else:
        test_features_ts = val_features_ts
        test_labels_ts = val_labels_ts

    logger.info(f"Extracted features: train={train_features_ts.shape}, val={val_features_ts.shape}, test={test_features_ts.shape if test_all_pred_noises_and_noises else 'N/A'}")
    logger.info(f"Train labels: {train_labels_ts.shape}, Val labels: {val_labels_ts.shape}, Test labels: {test_labels_ts.shape if test_all_pred_noises_and_noises else 'N/A'}")
    logger.info(f"Train features mean: {train_features_ts.mean().item()}, std: {train_features_ts.std().item()}")  # Debugging information for feature normalization
    logger.info(f"Val features mean: {val_features_ts.mean().item()}, std: {val_features_ts.std().item()}")  # Debugging information for feature normalization
    logger.info(f"Test features mean: {test_features_ts.mean().item()}, std: {test_features_ts.std().item()}")  # Debugging information for feature normalization
    ...
    eval_report_path = f"{config['output']}/eval_report_{config['experiment_name']}.json"
    with open(eval_report_path, 'w') as f:
        json.dump(eval_report, f)
    logger.info(f"Experiment '{config['experiment_name']}' completed. Evaluation report saved to {eval_report_path}")
...
def run_experiments(config):
    ...
    if need_denoise := bool(set(experiment_configs[0].keys()).intersection(CONFIG_ATTRIBUTES_REQUIRE_DENOISE)):
...
def run_generalisation_experiment(config):
    ...
    logger.info(f"Running generalisation experiment with {len(model_dirs)} model directories...")

    models_pipeline_and_data = {
        model_dir: run_denoise(base_config, train_dataloader, test_dataloader)
        for model_dir, (train_dataloader, test_dataloader) in zip(model_dirs, zip(train_dataloaders.values(), test_dataloaders.values()))
    }

    for this_model_dir, this_pnd in models_pipeline_and_data.items(): # pnd is pipeline_and_data
        for that_model_dir, that_pnd in models_pipeline_and_data.items(): # inner loop to iterate over each model directory
            this_pipeline, this_train_all_pred_noises_and_noises, this_train_all_labels, this_val_all_pred_noises_and_noises, this_val_all_labels = this_pnd
            _, _, _, that_val_all_pred_noises_and_noises, that_val_all_labels = that_pnd

            base_config['experiment_name'] = f"generalisation_{this_model_dir}_to_{that_model_dir}"

            logger.info(f"Running generalisation experiment from {this_model_dir} to {that_model_dir}...")
...
def normalise_config(config):
    ...
    if 'diffusion_percent' in config['base']:
        config['base']['diffusion_percent'] = convert_diffusion_percent_to_float(config['base']['diffusion_percent'])
    ...
    if IS_ON_GOOGLE_COLAB and 'output' in config['base']:
        config['base']['output'] = f"/content/drive/MyDrive/mypipeline_exps_2/{config['base']['output']}"
...
if __name__ == "__main__":
    ...
    logger.info(f"Loaded 1 generalisation experiment configuration and {len(other_configs)} other experiment configurations.")
